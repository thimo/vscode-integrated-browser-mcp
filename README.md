...
| `browser_press_key` | Press a key (Escape, Enter, arrow keys, F1-F12, or any printable character) via real keyboard input. Optional `selector` to focus first, `modifiers` for Alt/Control/Meta/Shift. Page listeners see every key; VS Code itself intercepts Escape, F-keys and most Ctrl/Cmd/Alt chords before the browser's native default runs, so Escape does not close a popover or `<dialog>` in the integrated browser (use `browser_eval` with `hidePopover()` / `requestClose()`). |

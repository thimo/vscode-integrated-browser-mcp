...
- `browser_markdown` now accepts `outputPath` (absolute). When set, the MCP server writes the extracted markdown to disk and returns only `Saved N bytes to <path>` — the body never flows through the agent's context. Built for bulk archival (scrape 50 doc pages, summarize externally) where the default return-the-body shape would balloon the agent's output token budget. Parent dirs are auto-created (`recursive: true`); existing files are overwritten; relative paths are rejected with a clear error. Mirrors `browser_screenshot`'s shape — visual artifacts and bulk text artifacts both belong on disk, not in context.
